...
<div class="relative w-4/5 mt-14 p-2 mx-auto max-w-md sm:p-4">
    <div class="relative p-4 bg-white rounded-lg shadow dark:bg-gray-800 sm:p-5">
        ...
        <form action="/worksheet/{{ $worksheet->id }}/{{ $material->id }}/material/update" method="post"
            ...
            <div class="grid gap-4 mb-4 sm:grid-cols-2">
                ...
                <div>
                    <label for="quantity" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Anyag
                        mennyisége</label>
                    <input type="number" name="quantity" id="quantity"
                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500"
                        value="{{ $material->quantity }}" min="1" max="100" required>
                    @error('quantity')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                </div>
        ...
        <form action="/worksheet/{{ $worksheet->id }}/{{ $material->id }}/material/destroy" method="post"
            class="mt-[-36px]">
            @csrf
            @method('delete')
