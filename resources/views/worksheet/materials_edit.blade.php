    ...
    <div class="relative p-4 bg-white rounded-lg shadow dark:bg-gray-800 sm:p-5">
        <div class="flex justify-between items-center pb-4 mb-4 rounded-t border-b sm:mb-5 dark:border-gray-600">
            <h3 class="text-3xl font-semibold text-gray-900 dark:text-white">
                Hozzáadott anyag szerkesztése
            </h3>
        </div>
        <form action="/worksheet/{{ $worksheet->id }}/{{ $material->id }}/material/update" method="post" enctype="multipart/form-data">
            @csrf
            @method('put')
            <div class="grid gap-4 mb-4 sm:grid-cols-2">
                <div>
                    ...
                    <input type="text" name="name" id="name"
                        ...
                        value="{{ $material_name }}" required="" disabled>
                    @error('name')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                </div>
                <div>
                    <label for="quantity" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Anyag mennyisége</label>
        ...
        <form action="/worksheet/{{ $worksheet->id }}/{{ $material->id }}/material/destroy" method="post" class="mt-[-36px]">
            @csrf
            @method('delete')
